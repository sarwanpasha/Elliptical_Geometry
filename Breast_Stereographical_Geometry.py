...
def stereographic_geometry(seq11, seq22):
    b=2*np.linalg.norm(seq11-seq22)
    c=np.linalg.norm(seq22)
    a=np.linalg.norm(seq11)
    
    d=b/np.sqrt((1+(a*a))*(1+(c*c)))
#     old haris
#     kernel_value = 2*np.arcsin(d/2)
#     new mine
    sigma = 1
...
#            K[i, j] = elliptic_geometry(seq1, seq2)
            K[i, j] = stereographic_geometry(seq1, seq2)
            K[j, i] = K[i, j]
